...
async function qualifyDesktop(browser, state) {
  const context = await browser.newContext({ viewport: { width: 1360, height: 920 }, locale: 'fr-FR', timezoneId: 'Europe/Paris', colorScheme: 'dark', reducedMotion: 'reduce', serviceWorkers: 'block', acceptDownloads: true })
  ...
  let idea = map.locator('.react-flow__node').filter({ hasText: 'Idée navigateur D08' }).first()
  ...
  const taskNode = map.locator('.react-flow__node').filter({ hasText: 'Tâche initiale D08' }).first(); idea = map.locator('.react-flow__node').filter({ hasText: 'Idée navigateur D08' }).first()
  ...
  await eventually(() => state.layoutWrites > writesBeforeGroup && Boolean(state.layouts.get(`mindmap.project.${ids.project}`)?.layout?.groups), 'D08: group layout was not persisted')

  const downloadPromise = page.waitForEvent('download'); await map.getByRole('button', { name: 'Exporter', exact: true }).click(); const download = await downloadPromise
  assert.equal(download.suggestedFilename(), `nevolium-mindmap-${ids.project}.json`)

  idea = page.locator('.mindmap-workspace:visible .react-flow__node').filter({ hasText: 'Idée navigateur D08' }).first(); await idea.click()
  await page.locator('.mindmap-workspace:visible .mindmap-conversion-bar').getByRole('button').click(); await eventually(() => state.conversions === 1, 'D08: idea conversion was not sent')
...
async function qualifyReload(browser, state) {
  ...
  const map = await openMindMap(page); await eventually(() => state.mindmapReads > beforeReads, 'D08: map was not reloaded')
...
async function qualifyPhone(browser) {
  ...
  const context = await browser.newContext({ viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true, locale: 'fr-FR', timezoneId: 'Europe/Paris', colorScheme: 'dark', reducedMotion: 'reduce', serviceWorkers: 'block' })
  await installApiMock(context, state); const page = await context.newPage(); const errors = []; page.on('pageerror', error => errors.push(error.stack || error.message))
...
try {
  ...
  const failure = { status: 'failed', error: error instanceof Error ? error.stack || error.message : String(error), recent_requests: state.requests.slice(-50) }
