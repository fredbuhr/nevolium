...
## D09 actif — pilote stable, acceptation d’usage bloquée
...
| Branche / PR | `fix/d09-guided-workspace`, [PR #96](https://github.com/fredbuhr/nevolium/pull/96) en brouillon ; tête distante `f7c4d04033a52e267aaa0e839286387bd7d9661e`. Extension Accueil/corpus committée localement `1c00a18`, précédée du complément `f07918d` et checkpoint `9005cee`. Publication explicitement autorisée par l’utilisateur, aucun merge ni déploiement |
...
| Canon | Extension locale : une projection PostgreSQL en lecture `GET /v1/mycelium/{type}/{id}` sur neuf types existants ; pagination 36/max80, droits à chaque extrémité ; aucune nouvelle table ni migration. Pilote : API D08 initiale toujours active |
...
| Prochaine action | Terminer la qualification de l’extension Accueil et corpus dans la branche PR #96 : publication autorisée réalisée (`6f20fdc`), corriger les quatre échecs de qualification navigateur du run `35110283457`, relancer Chromium puis inspecter les captures. Aucun go ni déploiement anticipé ; le prochain déploiement devra inclure Core et Web. D10 reste fermé |
...
| Correctif local | Accueil central utilisant le renderer organique partagé ; raccourcis par compte, dossiers, ordre, annulation, navigation transversale et retour au contexte ; ancien menu SVG retiré. Corpus fictif 5 projets/20 documents/12 fichiers/12 tâches/9 dépendances/32 relations, import public avec journal de reprise. Voir le correctif détaillé |
| Validation du correctif | Tête `f7c4d04033a52e267aaa0e839286387bd7d9661e` : **8/8 workflows PR déclenchés verts**, UI run `35101172713`, quatre jobs verts dont toutes les suites navigateur D05–D09. Création/contenu/brouillon/citations/restauration/EN/téléphone verts ; cas sans relations : 3 appartenances, 478 pixels animés, 0 en pause. Archives Knowledge et spatial téléchargées et empreintes vérifiées ; captures FR/EN/téléphone/petit réseau inspectées. Le complément d’en-tête ultérieur doit encore passer la CI |
...
| Publication autorisée | L’utilisateur autorise explicitement le 16 septembre l’envoi dans `fredbuhr/nevolium`, PR #96, et la poursuite de la qualification. Le refus auto-review antérieur est levé. Git CLI sans identifiants ; transfert par la connexion GitHub authentifiée, avec contrôle de l’arbre. PostgreSQL réel, import HTTP et huit workflows non-UI réussis sur `6f20fdc`. Première passe UI : quatre échecs de sélecteurs/attente dans les suites navigateur ; correctifs en cours |
| Validation locale du complément | Extension Accueil : TypeScript/build, contrats cockpit/locale/matière/appartenance/modèle d’accueil, Ruff F/E9, compilation SQL neuf types et contrat d’import HTTP passent. PostgreSQL non disponible ; Chromium et headless-shell officiels arrêtés par SIGTRAP au démarrage, suites nouvelles ajoutées en CI mais non exécutées. Pas de preuve visuelle du nouvel accueil |
