...
| Refs contrôlées le 2026-09-13 | `main` = `45b74baa3ddf8910f2aaa3d23c63f3e9bbedcf60` ; head de code D04 = `969fe668d08984b0e6aea38b8ba7f1ff18972b21`. Le diff depuis la base canonique vers `main` ne modifie que `PROJECT_STATE.md` |
| Dernier code déployé confirmé | Core `e3adbe648b245e2567970769e6a4bf4333b3e4a4` ; Worker `35303f2e3a5ea181a3b960f63bfaef1a12f4191c` ; LiteLLM `e4d886b9b32907135cb5faeb7737f09de27c6389` ; Web MCP `db3da89acfb041db75e6c7a2417a83dbafc6ce80` |
| Activation cible confirmée par sortie opérateur | LiteLLM reste sain avec `num_thread=2`, zéro retry et délai 210 s. Après un second contrôle à zéro workflow/réservation active, seul le Worker est recréé sans build, dépendance ni pull sur l'image `35303f2…` préalablement vérifiée. Les pollers Workflow et Activity portent la nouvelle identité `54a82541cbcd`. Core, LiteLLM, Ollama et Web MCP ne sont pas recréés ; services publics sains et image Worker `e4d886b…` conservée sous `rollback-e4d886b9b329` |
| Image Worker préparée, non active | `sha256:b14d17a97f3f295b3ad78d13ee16fe61da11f050c65d735bddbe711b0cc26b6b`, construite depuis le code `969fe66…`. Le module installé correspond au checkout et le contrat Research complet passe dans un conteneur en lecture seule sans réseau. Le Worker `35303f2…` reste actif et son image est conservée sous `rollback-35303f2e3a5e` ; le rollback `e4d886b…` reste également intact |
| Validation | `COLD-03` reste échouée et conservée avec 2 388 jetons. La Task unique `COLD-04` (`e1c81a97…`) reste elle aussi échouée et conservée avec 2 290 jetons, coût nul, réservation réglée, aucun outil ni artefact. Elle confirme un vrai appel Ollama froid à deux threads, puis échoue uniquement parce que l'objet JSON complet omet le `rationale` supérieur. Le correctif borné `969fe66…` préserve les appels, ajoute seulement une note d'audit déterministe pour ce champ absent et passe **10/10 workflows PR**, dont **5/5 jobs D04** ([run](https://github.com/fredbuhr/nevolium/actions/runs/34763676792)). Le snapshot cible `0|0|5|failed|1|2388|1|0|0|0|failed|1|2290|1|0|0|0|25|25|14|9|0|18` est identique avant et après le build ; aucun conteneur ni Task n'est créé |
| Prochaine action | Synchroniser ce checkpoint sans rebuild, vérifier de nouveau le head, l'inactivité par statuts terminaux, le snapshot canonique, les conteneurs et les deux tags de rollback. Arrêter proprement puis recréer le seul Worker sur l'image `b14d17a…` avec `--no-build --no-deps --pull never`. Prouver le module actif, les deux pollers, la stabilité des services adjacents et l'absence de nouvelle Task avant de préparer une mesure froide distincte `COLD-05` |
