...
| Refs contrôlées le 2026-09-13 | `main` = `45b74baa3ddf8910f2aaa3d23c63f3e9bbedcf60` ; head technique D04 = `35303f2e3a5ea181a3b960f63bfaef1a12f4191c`. Le diff depuis la base canonique vers `main` ne modifie que `PROJECT_STATE.md` |
| Dernier code déployé confirmé | Core `e3adbe648b245e2567970769e6a4bf4333b3e4a4` ; Worker et LiteLLM `e4d886b9b32907135cb5faeb7737f09de27c6389` ; Web MCP `db3da89acfb041db75e6c7a2417a83dbafc6ce80` |
| Activation cible confirmée par sortie opérateur | Avant arrêt : zéro workflow et réservation active, image précédente égale au tag `rollback-d78f45ac911d`, image préparée distincte et code gateway identique au checkout. LiteLLM recréé sans build avec `num_thread=2`, zéro retry, délai 210 s, modèle local exact et aucune clé externe. Worker recréé sans build sur la nouvelle image ; bornes 180/10 s et pollers Workflow/Activity vérifiés. Core non recréé et services publics sains |
| Validation | Activation deux threads prouvée, puis `NEVOLIUM-D04-RESEARCH-WEB-COLD-03` échoue au planning en 52,56 s. Ollama utilise réellement deux threads et termine l'appel froid en 30,65 s : 2 278 + 110 = 2 388 jetons, coût nul et réservation réglée. Le modèle renvoie une liste JSON complète sous fence Markdown au lieu de l'objet `ResearchPlan` ; aucun outil ni artefact n'est créé. Correctif `35303f2…` : **10/10 workflows PR réussis**, dont **5/5 jobs D04** ([run](https://github.com/fredbuhr/nevolium/actions/runs/34758531063)). Sur cible, `COLD-03` et son usage restent intacts ; l'image Worker corrigée est construite, distincte et son normaliseur est vérifié hors réseau. L'image active `e4d886b…` reste en fonctionnement et sous tag de rollback ; aucun service ni Task n'est recréé |
| Prochaine action | Synchroniser ce checkpoint documentaire puis, après un nouveau contrôle d'inactivité, recréer uniquement le Worker avec l'image corrigée déjà construite, sans build, dépendance ni pull. Vérifier l'identité de l'image, les pollers Workflow/Activity et les services publics avant toute nouvelle Task ; ne pas rejouer `COLD-03` |
