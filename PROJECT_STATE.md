...
## D09 ouvert — image du correctif d’affichage contrôlée, activation à effectuer

| Champ | État attesté |
|---|---|
| Base vérifiée | Main `faef129a28b1d09c1644654eab754f00304f68e8` après fusion de #99. La release KISS active reste `1af538b…`; le correctif d’affichage est construit et contrôlé mais pas encore activé |
...
| Gate autorisé | Refondation acceptée, [ADR-034](docs/decisions/ADR-034-human-first-refoundation.md) complète ADR-033. DOC-01 et activation D09-REC-01 terminées ; **D09-UX actif**, correctif #99 prêt à activer, D09 non accepté, D10 non commencé |
...
| Effets constatés lors de la préparation | Construction de l'image Web candidate et conteneur isolé supprimé. Core, Worker, Web-MCP et Web actif inchangés. Aucune activation, migration, import, mutation métier ou traitement IA |
...
| Prochaine action exécutable | Activer **Web seul** depuis la release exacte `faef129…` avec l'image déjà contrôlée `sha256:498873…`, sans reconstruction. Vérifier l'index interne, hôte et HTTPS public, l'OIDC et l'absence de changement des services non-Web. Retour automatique vers `sha256:2610523…` au premier échec |
...
| Recette suivante dans la même gate | Après activation Web contrôlée : revalider sur ordinateur les trois écarts corrigés, puis liste 2D, mode calme, téléphone ou tablette, et second compte pour l'isolation des épingles et de l'image personnelle |
...
| Accès et autorisations | Aucun accès SSH direct de l'agent. La prochaine activation Web reste une étape opérateur séparée ; aucun effet serveur n'a été produit par #99 ou ce checkpoint |

Commencer par l'activation Web seule et contrôlée. Après succès technique, effectuer la recette sur
ordinateur et arrêter au premier défaut important afin de le qualifier avant d'empiler d'autres
observations. Une capture peut être jointe si elle ne montre ni identité, contenu privé, jeton ni
