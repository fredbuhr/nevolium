...
## D09 ouvert — correctif mobile intégré, image Web à préparer

| Champ | État attesté |
|---|---|
| Base vérifiée | Code produit `5d31620f13ea5288425e0fcb9a9f419fa2cc2f65` après fusion de #100. Release de production active `/opt/nevolium/releases/faef129a28b1d09c1644654eab754f00304f68e8` ; le correctif mobile n'est pas encore construit ni activé |
| Branche / PR | [PR #100](https://github.com/fredbuhr/nevolium/pull/100) fusionnée par squash dans `5d31620…`. Aucune PR ouverte ; ne pas réutiliser `fix/d09-mobile-mycelium-space` ni `fix/d09-display-menu` |
| Gate autorisé | Refondation acceptée, [ADR-034](docs/decisions/ADR-034-human-first-refoundation.md) complète ADR-033. DOC-01 et activation D09-REC-01 terminées ; **D09-UX actif**, affichage ordinateur accepté, correctif tactile intégré mais non activé, isolation multi-compte suspendue, D09 non accepté, D10 non commencé |
...
| Correctif mobile intégré | #100 active une composition 3D dédiée au téléphone : introduction, légende, liste dupliquée et statut masqués ; navigation conservée compacte ; fiche bornée en panneau inférieur ; étiquettes réduites. 8/8 workflows réussis ; la qualification réserve au moins 55 % de hauteur tactile libre et limite la fiche à 45 %. [Preuve](https://github.com/fredbuhr/nevolium/pull/100#issuecomment-5706227430) |
| Prochaine action exécutable | Préparer une image **Web seule** depuis le commit exact `5d31620…`, la contrôler isolément avec les mêmes paramètres publics, puis activer séparément avec retour automatique vers l'image active `sha256:498873…`. Ne reconstruire ni Core, Worker ou Web-MCP ; aucune migration ni import |
...
| Recette suivante dans la même gate | Après préparation et activation Web contrôlées : refaire uniquement l'usage 3D sur téléphone, puis utiliser un second compte pour vérifier l'isolation des épingles, dispositions et image personnelle |
...
Commencer par préparer et contrôler isolément l'image Web du correctif mobile. Ne pas poursuivre la
recette multi-compte tant que ce correctif n'est pas activé et accepté sur le téléphone réel. Une capture peut être jointe si elle ne montre ni identité, contenu privé, jeton ni
