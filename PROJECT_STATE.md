...
| Dernier code déployé confirmé | Core `e3adbe648b245e2567970769e6a4bf4333b3e4a4` ; Worker `969fe668d08984b0e6aea38b8ba7f1ff18972b21` ; LiteLLM `e4d886b9b32907135cb5faeb7737f09de27c6389` ; Web MCP `db3da89acfb041db75e6c7a2417a83dbafc6ce80` |
| Activation cible confirmée par sortie opérateur | Checkout serveur `fb59fc4…` ; seul le Worker est recréé sans build, dépendance ni pull. Son nouveau conteneur `a04ca3056060…` sert le module installé `969fe66…` et possède les pollers Workflow et Activity de suffixe `@a04ca3056060`. Core, LiteLLM, Ollama et Web MCP gardent leurs identités. Snapshot SQL identique avant/après ; contrôles publics `200\|200\|401` ; aucune Task créée |
| Image Worker active et retours conservés | `sha256:b14d17a97f3f295b3ad78d13ee16fe61da11f050c65d735bddbe711b0cc26b6b`. Module résolu par Python dans `/app/.venv/lib/python3.12/site-packages/nevolium_worker/research_agent.py` et comparé au code ; contrat Research complet déjà validé hors réseau lors du build. Les tags `rollback-35303f2e3a5e` et `rollback-e4d886b9b329` restent intacts |
...
| Prochaine action | Synchroniser ce checkpoint documentaire sans redéploiement, recontrôler l'inactivité, les incidents conservés, le Worker actif et l'absence de Task/marqueur `COLD-05`. Décharger uniquement le modèle natif `qwen2.5:0.5b`, distinct de son nom LiteLLM `ollama/qwen2.5:0.5b`, puis créer le nouveau marqueur sans écraser un fichier existant. Ne lancer ensuite qu'une Task froide authentifiée distincte `NEVOLIUM-D04-RESEARCH-WEB-COLD-05` ; examiner son résultat avant l'essai chaud |
