...
| Validation | Activation deux threads prouvée, puis `NEVOLIUM-D04-RESEARCH-WEB-COLD-03` échoue au planning en 52,56 s. Ollama utilise réellement deux threads et termine l'appel froid en 30,65 s : 2 278 + 110 = 2 388 jetons, coût nul et réservation réglée. Le modèle renvoie une liste JSON complète sous fence Markdown au lieu de l'objet `ResearchPlan` ; aucun outil ni artefact n'est créé. Aucun conteneur ne redémarre ou subit d'OOM |
| Prochaine action | Valider en CI le normaliseur borné préparé sur la branche : retirer uniquement une fence JSON couvrant toute la réponse et envelopper une liste directe en `ResearchPlan`, sans nouvel appel modèle. Conserver les contrôles Pydantic, allowlist et schémas, puis déployer seulement le Worker et lancer une nouvelle Task froide distincte ; ne pas rejouer `COLD-03` |
